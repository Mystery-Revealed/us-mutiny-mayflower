...
// and inspects the emit instructions it returns. No sockets involved. Mutiny on
// the Mayflower is single-role solo (no pick, no branch, no AI rival), so these
// focus on the solo lifecycle, the one class-wide group, and the accuracy path.
import test from 'node:test';
import assert from 'node:assert/strict';
import { GameManager } from '../src/GameManager.js';
import game, { phasesFor, SIDE } from '../src/games/usMutinyMayflower.js';
...
function playAllRight(manager, joinCode, studentId) {
  ...
  for (let i = 0; i < game.totalActions; i++) {
    ...
    assert.ok(!last.error, `scene ${i}: ${last.error}`);
  }
  return last;
}
...
test('the default game is Mutiny on the Mayflower', () => {
  const manager = new GameManager();
  const joinCode = makeSession(manager);
  assert.equal(manager.registry.get(joinCode).gameId, 'us-mutiny-mayflower');
});
...
test('a student joins, the match begins on join, and all-right earns 100% and "41 Names"', () => {
  const manager = new GameManager();
  const joinCode = makeSession(manager);
  ...
  assert.equal(begin[0].payload.side, 'bradford');
  ...
  assert.equal(end.length, 1, 'match ends after 8 scenes');
  assert.equal(end[0].payload.you.accuracy, 100);
  assert.equal(end[0].payload.you.ending.key, 'names');
  assert.match(end[0].payload.you.debrief, /Wampanoag/, 'epilogue present');
...
test('every resolution carries a verdict but never the running answer key', () => {
  ...
  assert.equal(typeof resolution.stepIndex, 'number', 'the client can key the money moment on stepIndex');
});
...
test('class accuracy is one class-wide group (spec §1: no pick)', () => {
  ...
  const roster = manager.roster(manager.registry.get(joinCode));
  assert.equal(roster.classAccuracy.bradford.count, 2, 'both students in one group');
  assert.equal(roster.classAccuracy.bradford.average, 100);
});
...
test('a wrong-kind move is rejected (every scene is a decision)', () => {
  const manager = new GameManager();
  const joinCode = makeSession(manager);
  const res = join(manager, joinCode, 'Ana');
...
test('rejoin returns a full snapshot of the live scene', () => {
  ...
  const joinCode = makeSession(manager);
  const res = join(manager, joinCode, 'Ana');
  playRight(manager, joinCode, res.studentId); // one scene done; another pending
