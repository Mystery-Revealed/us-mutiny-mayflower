...
// download the PDF → End Session (deletes everything from memory). Mutiny on the
// Mayflower is single-role solo (no pick), so students start on their own as soon
// as they're approved — no pairing, and one class group instead of a per-pick split.
...
const GROUP = 'bradford';
const TEKS = 'TEKS 8.3B, 8.1B, 8.15A, 8.19A';
  ...
  if (phase === 'gate') {
    return (
      <div className="app teacher-app">
        ...
        <p className="muted">Mutiny on the Mayflower · {TEKS}</p>
  ...
  return (
    <div className="app teacher-app">
      <header className="row">
        <div>
          <h1>Teacher Command Center</h1>
          <p className="muted">Mutiny on the Mayflower · session-only data · download the PDF before you end</p>
        </div>
      ...
      {pending.length > 0 && (
        <div className="card">
          ...
          <p className="muted">Approve a name and that student steps onto the deck right away.</p>
          {pending.map((s) => (
      ...
      <div className="card">
        <div className="class-summary one">
          <div className="summary-stat wide">
            ...
            <div className="muted">Mayflower — {students.length} students — {cls.count} completed</div>
          </div>
        </div>
      </div>
...
function downloadPdf(roster, cls) {
  ...
  doc.setFontSize(16);
  doc.text('Mutiny on the Mayflower — Session Report', 14, 16);
  doc.setFontSize(10);
  doc.setTextColor(90);
  ...
  doc.text('8th Grade U.S. History · Mutiny on the Mayflower · TEKS 8.3B, 8.1B, 8.15A, 8.19A', 14, doc.internal.pageSize.getHeight() - 8);

  doc.save(`mutiny-on-the-mayflower_${roster.joinCode}_${date}.pdf`);
}
